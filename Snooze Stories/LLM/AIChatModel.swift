...
final class AIChatModel: ObservableObject {
    ...
    public init(){
        chat = nil
        self.modelURL = Bundle.main.path(forResource: "Phi-3-mini-128k-instruct.IQ4_NL", ofType: "gguf") ?? "Model not found"
    }
...
//        self.model_name = model_name        
        ...
        print("AI messages = \(self.messages)")
    ...
    public func process_predicted_str(_ str: String, _ time: Double, _ message: inout Message) -> Bool {
        ...
        if check && self.chat?.flagExit != true && self.chat_name == self.chat?.chatName {
            ...
            let pattern = "\\s*<\\|assistant\\|>\\s*"
            let cleanedStr = str.replacingOccurrences(of: pattern, with: "", options: .regularExpression)  
            
            // Remove the first character if it is a space
            if !message.text.isEmpty && message.text.first == " " {
                message.text.removeFirst()
            }

            message.text += cleanedStr
            self.AI_typing += 1
