...
func create_chat_get_name(_ in_options: Dictionary<String, Any>, edit_chat_dialog: Bool = false, chat_name: String = "", save_as_template: Bool = false) -> (Bool, String?) {
    do {
        var options: Dictionary<String, Any> = [:]
        for (key, value) in in_options {
            print("\(key) : \(value)")
        ...
        let today = Date()
        let timeInterval = today.timeIntervalSince1970
        _ = "_" + String(Int(timeInterval))
        let fname = chat_name
//        if edit_chat_dialog {
//            fname = chat_name
//        } else {
//            fname = options["title"]! as! String + salt + ".json"
//            print("Potentially the name: \(fname)")
//        }
//        if save_as_template {
//            fname = chat_name
//        }
        let path = destinationURL.appendingPathComponent(fname)
        try jsonData.write(to: path)
        return (true, fname) // Successful creation with the file name
    } catch {
        print(error)
        return (false, nil) // Failed creation, no file name
    }
}
...
func save_image_from_library_to_cache(_ image: UIImage?) -> String?{
    do {
        ...
        let framework: UIImage.ResizeFramework = .accelerate
        _ = Date()
...
func copyModelToSandbox(url: URL, dest: String = "models") -> String? {
    ...
    do {
        try fileManager.createDirectory(at: destinationURL, withIntermediateDirectories: true, attributes: nil)
        let newURL = destinationURL.appendingPathComponent(url.lastPathComponent)
        ...
        print("Failed to copy model to sandbox: \(error)")
        return nil
    }
}
